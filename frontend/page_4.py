...
def check():
    ...
    for i, ticket_data in enumerate(final_ticket):
        ...
        Label(fr, text='\n').grid(row=8, column=1)
        Label(fr, text="\tTotal amount " + str(amount) + " to be paid at the time of boarding").grid(row=9, column=1, columnspan=5)

        fr.grid(row=i+6, column=2)
